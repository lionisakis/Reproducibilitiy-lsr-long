...
parser.add_argument(
    "--inp", type=str, default="data/msmarco_doc/splits_psg/part01", help="Inpput file"
)
parser.add_argument(
    "--cp",
    type=str,
    default="lsr42/qmlp_dmlm_msmarco_distil_kl_l1_0.0001",
...
if not Path(args.cp).is_dir():
    ...
    try:
        snapshot_download(repo_id=args.cp, revision="main", cache_dir=args.cp)
    except:
        raise Exception("wrong model's checkpoint: {model_dir_or_name}")

# if config file is not in the folder of args.cp
if not Path(os.path.join(args.cp, "class_config.json")).exists():
    args.cp = os.path.join(args.cp, os.listdir(args.cp)[0])
    args.cp = os.path.join(args.cp, "snapshots")
    args.cp = os.path.join(args.cp, os.listdir(args.cp)[0])
