...
class DualSparseEncoder(nn.Module):
    ...
    def __init__(
        ...
        print("self.global_model device",next(self.global_model.parameters()).device)
        if disable_gradient:
        ...
        self.linear_sum = nn.Linear(len(self.window_sizes) * 2 - 1 + 2 , 1)
    ...
    def _score_pairs(
        ...
        forward_score = trans_matrix_forward.max(dim=2).values.sum(dim=1).unsqueeze(1)
        backwards_score = trans_matrix_backwards.max(dim=2).values.sum(dim=1).unsqueeze(1)            

        max_psg_scores = torch.cat((max_psg_scores,forward_score, backwards_score),dim=1)
