...
class DualSparseEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.global_model = global_encoder
        if disable_gradient:
    ...
    def encode_docs(self, **docs):
        ...
        return self.doc_encoder(**docs, return_raw=True, output_last_hidden_state=True)
    ...
    def score_pairs(self, queries, psgs, psg_offset):
        ...
        d_logits,hidden_states = self.encode_docs(**psgs)
        return self._score_pairs(
            q_tok_ids, q_tok_weights, d_tok_ids, d_logits, hidden_states, psg_offset
        )

    def _score_pairs(
        self, q_tok_ids, q_tok_weights, d_tok_ids, d_logits, hidden_states, psg_offset
    ):
        ...
        trans_matrix_forward, trans_matrix_backwards = self.global_model(hidden_states, psg_offset, q_tok_weights,q_tok_ids, self.doc_encoder)
        
        trans_matrix_forward_list=[]
        trans_matrix_backwards_list=[]
        idx_psg=1
        idx_global=0
        idx_psg_global=0
        for idx in range(batch_size):
            trans_matrix_forward_list.append(trans_matrix_forward[idx_global,:,idx_psg_global])
            trans_matrix_backwards_list.append(trans_matrix_backwards[idx_global,:,idx_psg_global])
            idx_psg_global+=1
            if idx+1 == psg_offset[idx_psg]: 
                idx_global+=1
                idx_psg+=1
                idx_psg_global=0
        
        trans_matrix_forward = torch.stack(trans_matrix_forward_list,dim=0).unsqueeze(-1)
        trans_matrix_backwards = torch.stack(trans_matrix_backwards_list,dim=0).unsqueeze(-1)
        # add trans_matrix_forward and trans_matrix_backwards to trans_matrix
        trans_matrix = torch.cat([trans_matrix,trans_matrix_forward,trans_matrix_backwards],dim=2)
    ...
    def save_pretrained(self, model_dir):
        ...
        global_dir_or_name = f"{model_dir}/global_encoder"
        os.makedirs(global_dir_or_name, exist_ok=True)
        self.global_model.save_pretrained(global_dir_or_name)
    ...
    def from_pretrained(cls, model_dir_or_name):
        ...
        if not Path(model_dir_or_name).is_dir():
            ...
            try:
                snapshot_download(
                    repo_id=model_dir_or_name, local_dir=model_dir_or_name
                )
            except:
        ...
        if not Path(os.path.join(model_dir_or_name, "class_config.json")).exists():
            ...
            model_dir_or_name = os.path.join(model_dir_or_name, "snapshots")
            model_dir_or_name = os.path.join(
                model_dir_or_name, os.listdir(model_dir_or_name)[0]
            )
        ...
        encoder_name_or_dir = f"{model_dir_or_name}/global_encoder"
        ...
        if Path(encoder_name_or_dir).is_dir():
            global_encoder = Global_Encoder.from_pretrained(encoder_name_or_dir)
        else:
            from lsr import config

            configuration = config.config
            global_encoder = Global_Encoder(
                configuration.model.global_encoder.encoder_name_or_dir,
                configuration.model.global_encoder.embedding_dim,
                configuration.model.global_encoder.hidden_size_encoder,
                configuration.model.global_encoder.num_layers,
            )
        ...
        if config_path.is_file():
            ...
            sdm_config = {}
        model = cls(query_encoder, doc_encoder, global_encoder, **sdm_config)
